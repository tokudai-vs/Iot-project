...
function initializeValues() {
  fetch("data.json")
    ...
    .then(data => {
      ...
      // init ac value
      LR.DrawingRoom.AC.AC1.temperature = data.DrawingRoom.AC.AC1.temperature

      // AC switch
      if (data.DrawingRoom.AC.AC1.status == "on") {
        document.querySelector("#acDisplay").innerText = data.DrawingRoom.AC.AC1.temperature
      ...
      no_of_lights = Object.keys(data.DrawingRoom.Light).length;
      var s = "";
      ...
      for (var i = 1; i <= no_of_lights; i++) {
        LR.DrawingRoom.Light["L" + i].brightness = data.DrawingRoom.Light["L" + i].brightness;
        ...
        if (data.DrawingRoom.Light["L" + i].status == "on") {
          document.getElementById(i).value = data.DrawingRoom.Light["L" + i].brightness;
          document.getElementById("togBtnLight" + i).checked = true;
          document.getElementById("lightSliderOut" + i).innerText = data.DrawingRoom.Light["L" + i].brightness;
      ...
      no_of_fans = Object.keys(data.DrawingRoom.Fan).length;
      ...
      //init fan values
      // document.querySelector("#FanSpeed").innerText = data.DrawingRoom.Fan.F1.speed
      for (var i = 1; i <= no_of_fans; i++) {
        document.getElementById("roundSliderFan" + i).classList.add("round")

        if (data.DrawingRoom.Fan["F"+i].speed == 1) {
          document.getElementById("F" + i +'btn1').classList.add("active")
        }
        else if (data.DrawingRoom.Fan["F"+i].speed == 2) {
          document.getElementById("F" + i +'btn2').classList.add("active")
        }
        else if (data.DrawingRoom.Fan["F"+i].speed == 3) {
          document.getElementById("F" + i +'btn3').classList.add("active")
        }
        else if (data.DrawingRoom.Fan["F"+i].speed == 4) {
        ...
        if(data.DrawingRoom.Fan["F"+i].status == "on"){
...
function increaseValue() {
  ...
  if (check) {
    ...
    value++;
    // LR.DrawingRoom.AC.AC1.temperature = value;
    document.querySelector("#acDisplay").innerText = value;
    eel.valueFromJS("DrawingRoom", "AC", "AC1", "temperature", value)
  }
  else {
...
function toggleCheck() {
  ...
  if (check) {
    document.querySelector("#acDisplay").innerText = LR.DrawingRoom.AC.AC1.temperature
    document.getElementById("decrease").style.opacity = 1;
    document.getElementById("increase").style.opacity = 1;
    // LR.DrawingRoom.AC.status = "on";
    eel.valueFromJS("DrawingRoom", "AC", "AC1", "status", "on")
    ...
    document.getElementById("decrease").style.opacity = 0.2;
    document.getElementById("increase").style.opacity = 0.2;
    // LR.DrawingRoom.AC.status = "off";
    eel.valueFromJS("DrawingRoom", "AC", "AC1", "status", "off")
...
function te(value, id) {
  ...
  if (check) {
    ...
    eel.valueFromJS("DrawingRoom", "Light", "L" + id, "brightness", parseInt(value))
...
function toggleCheckLight(id, index) {
  ...
  if (check) {
    ...
    document.getElementById("lightSliderOut" + index).value = LR.DrawingRoom.Light["L" + index].brightness;
    document.getElementById(index).value = LR.DrawingRoom.Light["L" + index].brightness;
    var sliderDiv = document.getElementById("lightSliderOut" + index);
    sliderDiv.innerHTML = parseInt(sliderDiv.value);
    eel.valueFromJS("DrawingRoom", "Light", "L" + index, "status", "on");
    ...
    eel.valueFromJS("DrawingRoom", "Light", "L" + index, "status", "off");
...
function fanSpeedController(id, index) {
  ...
  LR.DrawingRoom.Fan["F" + index].speed = parseInt(document.getElementById(id).innerText);
  eel.valueFromJS("DrawingRoom", "Fan", "F" + index, "speed", LR.DrawingRoom.Fan["F" + index].speed);
}
...
function toggleCheckFan(id, index) {
  ...
  if(checkFan){
    ...
    eel.valueFromJS("DrawingRoom", "Fan", "F" + index, "status", "on");
    ...
    eel.valueFromJS("DrawingRoom", "Fan", "F" + index, "status", "off");
...
function addCode() {
  ...
  //default values of newly added lights
  var temp = {
    "status": "off",
    "brightness": 47
  }
  LR.DrawingRoom.Light['L' + no_of_lights] = temp;
  console.log(LR)
  eel.updateJSON('DrawingRoom','Light', 'L' + no_of_lights, temp);
  location.reload();
}

//below function is used to add FAN
function addCodeFan() {
  no_of_fans += 1;
  document.getElementById("fan").innerHTML +=
...
// default values of newly added componets
  var temp = {
    "status": "off",
    "speed": 3
  }
  LR.DrawingRoom.Fan['F' + no_of_fans] = temp;
  console.log(LR)
  eel.updateJSON('DrawingRoom','Fan', 'F' + no_of_fans, temp);
  location.reload();
}


//below function is used to reload the page
