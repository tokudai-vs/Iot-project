...
function initializeValues() {
  fetch("data.json")
    ...
    .then(data => {
      ...
      // init ac value
      LR.BedRoom.AC.AC1.temperature = data.BedRoom.AC.AC1.temperature

      // AC switch
      if (data.BedRoom.AC.AC1.status == "on") {
        document.querySelector("#acDisplay").innerText = data.BedRoom.AC.AC1.temperature
      ...
      no_of_lights = Object.keys(data.BedRoom.Light).length;
      var s = "";
      ...
      for (var i = 1; i <= no_of_lights; i++) {
        LR.BedRoom.Light["L" + i].brightness = data.BedRoom.Light["L" + i].brightness;
        ...
        if (data.BedRoom.Light["L" + i].status == "on") {
          document.getElementById(i).value = data.BedRoom.Light["L" + i].brightness;
          document.getElementById("togBtnLight" + i).checked = true;
          document.getElementById("lightSliderOut" + i).innerText = data.BedRoom.Light["L" + i].brightness;
      ...
      no_of_fans = Object.keys(data.BedRoom.Fan).length;
      ...
      for (var i = 1; i <= no_of_fans; i++) {
        ...
        if(data.BedRoom.Fan["F"+i].status == "on"){
...
function increaseValue() {
  ...
  if (check) {
    ...
    value++;
    // LR.BedRoom.AC.AC1.temperature = value;
    document.querySelector("#acDisplay").innerText = value;
    eel.valueFromJS("BedRoom", "AC", "AC1", "temperature", value)
  }
  else {
...
function toggleCheck() {
  ...
  if (check) {
    document.querySelector("#acDisplay").innerText = LR.BedRoom.AC.AC1.temperature
    document.getElementById("decrease").style.opacity = 1;
    document.getElementById("increase").style.opacity = 1;
    // LR.BedRoom.AC.status = "on";
    eel.valueFromJS("BedRoom", "AC", "AC1", "status", "on")
    ...
    document.getElementById("decrease").style.opacity = 0.2;
    document.getElementById("increase").style.opacity = 0.2;
    // LR.BedRoom.AC.status = "off";
    eel.valueFromJS("BedRoom", "AC", "AC1", "status", "off")
...
function te(value, id) {
  ...
  if (check) {
    ...
    eel.valueFromJS("BedRoom", "Light", "L" + id, "brightness", parseInt(value))
...
function toggleCheckLight(id, index) {
  ...
  if (check) {
    ...
    document.getElementById("lightSliderOut" + index).value = LR.BedRoom.Light["L" + index].brightness;
    document.getElementById(index).value = LR.BedRoom.Light["L" + index].brightness;
    var sliderDiv = document.getElementById("lightSliderOut" + index);
    sliderDiv.innerHTML = parseInt(sliderDiv.value);
    eel.valueFromJS("BedRoom", "Light", "L" + index, "status", "on");
    ...
    eel.valueFromJS("BedRoom", "Light", "L" + index, "status", "off");
...
function fanSpeedController(id, index) {
  ...
  LR.BedRoom.Fan["F" + index].speed = parseInt(document.getElementById(id).innerText);
  eel.valueFromJS("BedRoom", "Fan", "F" + index, "speed", LR.BedRoom.Fan["F" + index].speed);
}
...
function toggleCheckFan(id, index) {
  ...
  if(checkFan){
    ...
    eel.valueFromJS("BedRoom", "Fan", "F" + index, "status", "on");
    ...
    eel.valueFromJS("BedRoom", "Fan", "F" + index, "status", "off");
...
function addCode() {
  ...
  LR.BedRoom.Light['L' + no_of_lights] = temp;
  console.log(LR)
  eel.updateJSON('BedRoom','Light', 'L' + no_of_lights, temp);
  location.reload();
}
...
function addCodeFan() {
  ...
  LR.BedRoom.Fan['F' + no_of_fans] = temp;
  console.log(LR)
  eel.updateJSON('BedRoom','Fan', 'F' + no_of_fans, temp);
  location.reload();
}
